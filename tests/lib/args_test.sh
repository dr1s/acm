...
assert_eq "/project/conf/wowserver.conf" "$(find_config_arg /project)" "default config path"
assert_eq "/project/./conf/solo.conf" "$(find_config_arg /project --clean ./conf/solo.conf)" "relative config"
assert_eq "/etc/wow.conf" "$(find_config_arg /project /etc/wow.conf)" "absolute config"
assert_eq "/project/./conf/last.conf" "$(find_config_arg /project ./conf/first.conf --clean ./conf/last.conf)" "last config wins"
assert_eq "/project/./conf/second.conf" "$(find_config_arg /project ./conf/first.conf ./conf/second.conf --skip-build)" "last config wins with multiple"
assert_eq "/project/./conf/middle.conf" "$(find_config_arg /project --clean ./conf/middle.conf --skip-build)" "config in middle"
